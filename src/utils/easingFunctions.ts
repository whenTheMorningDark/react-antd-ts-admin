...
export const easeInOutExpo: IeasingFunction = (elps, from, to, dr) => {
  ...
  if ((elps /= dr / 2) < 1) {
    return to / 2 * 2 ** (10 * (elps - 1)) + from;
  }
  return to / 2 * (-(2 ** (-10 * --elps)) + 2) + from;
};
...
export const easeInElastic: IeasingFunction = (elps, from, to, dr) => {
  ...
  return (
    -(a * 2 ** (10 * (elps -= 1)) * Math.sin((elps * dr - s) * (2 * Math.PI) / p))
    + from
  );
};
...
export const easeOutElastic: IeasingFunction = (elps, from, to, dr) => {
  ...
  return a * 2 ** (-10 * elps) * Math.sin((elps * dr - s) * (2 * Math.PI) / p) + to + from;
};
...
export const easeInOutElastic: IeasingFunction = (elps, from, to, dr) => {
  ...
  if (elps < 1) {
    return (
      -0.5 * (a * 2 ** (10 * (elps -= 1)) * Math.sin((elps * dr - s) * (2 * Math.PI) / p))
      + from
    );
  }
  return (
    a * 2 ** (-10 * (elps -= 1)) * Math.sin((elps * dr - s) * (2 * Math.PI) / p) * 0.5
    + to + from
  );
};
